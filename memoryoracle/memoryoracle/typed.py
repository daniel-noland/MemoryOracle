...
class Typed(tracked.Tracked):
    ...
    @staticmethod
    def _register_type_handler(cls):

        if not isinstance(cls, Typed):
            raise ValueError("Type handler must be a Typed object")

        if Typed._type_lookup(cls._type_handler_code) is None:
    ...
    @staticmethod
    def _type_lookup(code):
        return _typeCodeMap.get(code)
